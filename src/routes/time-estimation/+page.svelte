<!-- TODO: Add essay assignment time estimation -->
...
<script>
    ...
    const menuOptions = ["Math", "Chemistry", "Physics", "Biology", "Programming", "Writing", "Essay", "Other"];
    ...
    function parse(pdf) 
    {
        // I'm using regular expressions here to clean up the file
        const rules = [
    ...
    {
        // Rough estimate of how long it takes to do a question
        const avg_time = {
                    "math": 4,
                    "chemistry": 5,
                    "physics": 5,
        ...
        // Calculate the base time it takes to complete assignment, including fatigue and barring confidence
        let base_time = 0;

        for (let i = 1; i < questionCount + 1; i++) {
            let fatigue_multiplier = 1 + fatigue_sensitivity[subject] * Math.sqrt(i);
            fatigue_multiplier = Math.min(fatigue_multiplier, 1.5); // Cap off fatigue at a certain point
        ...
        confidence_factor = Math.min(Math.max(confidence_factor, 0.25), 2.5); // Cap off confidence at a certain point

        // Expected time, assuming the all of the time estimates are 100% accurate (which they likely aren't)
    ...
    $effect(() => {
        ...
        const conf = (confidence ?? 50) / 100;

        estimation = estimateTime(processedText, subj, conf);
        ...
        <div class="w-full lg:w-1/2 flex flex-col">
            ...
            <div class="bg-[#fdfd96] border-2 border-black border-t-0 lg:border-l-0 lg:border-t-2 lg:rounded-tr-2xl shadow-[4px_4px_0px_0px_#000]">
                ...
                <div class="grid grid-cols-[auto_1fr] items-center gap-3 px-4 sm:px-6 pt-6 sm:pt-14">
                    ...
                    <div class="h-10 w-full max-w-55 bg-[#c4a1ff] border-2 border-black flex justify-between items-center px-2">
                        {#if subject === null}
            ...
            {#if showDropdown}
                <div class="w-full relative z-50">
                    {#each menuOptions as option}
                        <button 
                            type="button" 
                            class="w-full text-left p-2 hover:bg-gray-200 border-t-2 border-x-2 last:border-b-2 border-black bg-white"
            ...
            <div class="bg-[#bafcf2] border-2 border-black border-t-0 lg:border-l-0 lg:rounded-br-2xl shadow-[4px_4px_0px_0px_#000] flex flex-col min-h-62.5 sm:min-h-75">
                    ...
                    {#if estimation}
                        <div class="p-2 bg-white border-2 border-black rounded-xl font-mono text-center text-sm sm:text-base">
                            <p>
                                ...
                                Expected Time: {estimation[1]} minutes</b>
                            </p>
                        </div>
                    {:else if file}
                        <div class="p-2 bg-white border-2 border-black rounded-xl font-mono text-center text-sm sm:text-base">
                ...
                <form method="post" enctype="multipart/form-data" class="mt-auto flex flex-col sm:flex-row justify-center items-center gap-3 p-3" onsubmit={resetForm}>                   
